// ui.js — uploader + controls with auto-rotate, duplicate removal, manual rotation, drag-to-reposition (WebM-only)
...
export async function bootstrap(){
  ...
  els.exportBtn.onclick = async ()=>{
    ...
    const toDisable = [els.exportBtn, els.playBtn, els.pauseBtn, els.stopBtn, els.adjustBtn];
    toDisable.forEach(b=> b.disabled = true);
    els.dlArea.innerHTML = 'Exporting WebM…';

    try{
      await exporter.exportWebM();
      els.dlArea.innerHTML = 'WebM saved.';
    }catch(e){
      console.error(e);
